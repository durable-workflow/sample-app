...
final class PolyglotComposeContractTest extends TestCase
{
    ...
    public function test_polyglot_compose_artifacts_come_from_resolved_environment(): void
    {
        ...
        $this->assertSame($resolvedPhpSdkVersion, $waterlineBuildArgs['DURABLE_WORKFLOW_PHP_SDK_VERSION'] ?? null);
    ...
    public function test_sample_app_compose_can_build_against_resolved_php_artifacts(): void
    {
        ...
        $this->assertStringContainsString('locked_package_version durable-workflow/sdk', $installScript);
        ...
        $this->assertStringContainsString('composer update durable-workflow/sdk durable-workflow/workflow durable-workflow/waterline', $installScript);
        $this->assertStringContainsString(
            <<<'SH'
...
SH,
    ...
    public function test_polyglot_smoke_installs_published_cli_and_configures_waterline(): void
    {
        ...
        foreach ([
            ...
            'durable-workflow/workflow' => 'https://github.com/durable-workflow/workflow.git',
            'durable-workflow/waterline' => 'https://github.com/durable-workflow/waterline.git',
        ] as $package => $sourceUrl) {
            $packageVersion = $composerJson['require'][$package] ?? null;
            $this->assertIsString($packageVersion);
            $this->assertMatchesRegularExpression('/^2\.0\.0-rc\.\d+$/', $packageVersion);
            $this->assertIsArray($lockedPackages[$package] ?? null);
            $this->assertSame($packageVersion, $lockedPackages[$package]['version'] ?? null);
            $this->assertSame(
                $packageVersion,
                $lockedPackages[$package]['extra']['durable-workflow']['product-train'] ?? null,
            );
            $this->assertSame($sourceUrl, $lockedPackages[$package]['source']['url'] ?? null);
            $sourceReference = $lockedPackages[$package]['source']['reference'] ?? null;
            $this->assertIsString($sourceReference);
            $this->assertMatchesRegularExpression('/^[0-9a-f]{40}$/', $sourceReference);
            $this->assertSame($sourceReference, $lockedPackages[$package]['dist']['reference'] ?? null);
            $this->assertStringContainsString(
                $sourceReference,
                $lockedPackages[$package]['dist']['url'] ?? '',
            );
            $this->assertSame(
                'https://packagist.org/downloads/',
                $lockedPackages[$package]['notification-url'] ?? null,
            );
        }
        ...
        $this->assertSame(
            '${DURABLE_WORKFLOW_PHP_SDK_PIN:-}',
            $services['waterline']['build']['args']['DURABLE_WORKFLOW_PHP_SDK_PIN'] ?? null,
        );
        $this->assertSame(
            $this->requiredResolvedEnv('DURABLE_WORKFLOW_PHP_SDK_VERSION'),
            $services['waterline']['build']['args']['DURABLE_WORKFLOW_PHP_SDK_VERSION'] ?? null,
        );
        $this->assertSame(
            $this->requiredResolvedEnv('DURABLE_WORKFLOW_WORKFLOW_VERSION'),
    ...
    {
        $packageAssets = $this->directoryFileHashes(
            $this->repoPath('vendor/durable-workflow/waterline/public'),
        );

        foreach ([
            'public/vendor/waterline',
            'microservice/public/vendor/waterline',
        ] as $publishedDirectory) {
            $this->assertSame(
                $packageAssets,
                $this->directoryFileHashes($this->repoPath($publishedDirectory)),
                $publishedDirectory,
            );
        }
    }
    ...
    public function test_polyglot_artifact_resolver_loads_exact_release_candidate_tuple(): void
    {
        $assignments = $this->resolveArtifactAssignments([
        ...
        $this->assertSame('dw=='.$componentVersion, $assignments['DURABLE_WORKFLOW_CLI_PIN'] ?? null);
        $this->assertSame($componentVersion, $assignments['DURABLE_WORKFLOW_PYTHON_SDK_VERSION'] ?? null);
        $this->assertSame($componentVersion, $assignments['DURABLE_WORKFLOW_RUST_SDK_VERSION'] ?? null);
        $this->assertSame($componentVersion, $assignments['DURABLE_WORKFLOW_PHP_SDK_VERSION'] ?? null);
        $this->assertSame('durable-workflow/sdk:'.$componentVersion.'@beta', $assignments['DURABLE_WORKFLOW_PHP_SDK_PIN'] ?? null);
        $this->assertSame($componentVersion, $assignments['DURABLE_WORKFLOW_WORKFLOW_VERSION'] ?? null);
        $this->assertSame('durable-workflow/workflow:'.$componentVersion.'@beta', $assignments['DURABLE_WORKFLOW_WORKFLOW_PIN'] ?? null);
        $this->assertSame($componentVersion, $assignments['DURABLE_WORKFLOW_WATERLINE_VERSION'] ?? null);
    ...
    {
        $assignments = $this->resolveArtifactAssignments([
            'DURABLE_WORKFLOW_CURRENT_ARTIFACT_TUPLE_URL' => 'file://'.$this->repoPath('tests/Fixtures/release-candidate-artifact-tuple.json'),
        ], false);
        $artifactResolver = (string) file_get_contents($this->repoPath('scripts/resolve-current-artifacts.sh'));

        $this->assertSame('durableworkflow/server:2.0.0-rc.2', $assignments['DURABLE_SERVER_IMAGE'] ?? null);
        $this->assertSame('2.0.0-rc.1', $assignments['DURABLE_WORKFLOW_CLI_VERSION'] ?? null);
        $this->assertSame('2.0.0-rc.1', $assignments['DURABLE_WORKFLOW_PYTHON_SDK_VERSION'] ?? null);
        $this->assertSame('2.0.0-rc.1', $assignments['DURABLE_WORKFLOW_RUST_SDK_VERSION'] ?? null);
        $this->assertSame('2.0.0-rc.1', $assignments['DURABLE_WORKFLOW_PHP_SDK_VERSION'] ?? null);
        $this->assertSame('2.0.0-rc.1', $assignments['DURABLE_WORKFLOW_WORKFLOW_VERSION'] ?? null);
        $this->assertSame('2.0.0-rc.1', $assignments['DURABLE_WORKFLOW_WATERLINE_VERSION'] ?? null);
        $this->assertStringContainsString('https://durable-workflow.com/docs-page-release-audit.json', $artifactResolver);
        $this->assertStringContainsString('must expose one 2.0 beta or release-candidate channel', $artifactResolver);
    ...
    public function test_polyglot_artifact_resolver_keeps_pinned_tuple_explicit(): void
    {
        ...
        $this->assertSame('durable-workflow.sample-app.polyglot-qualified-artifact-tuple', $tuple['schema'] ?? null);
        $this->assertSame(1, $tuple['schemaVersion'] ?? null);
        foreach ($artifacts as $version) {
            $this->assertIsString($version);
            $this->assertMatchesRegularExpression('/^2\.0\.0-rc\.\d+$/', $version);
        }

        $this->assertSame('durableworkflow/server:'.$artifacts['server'], $assignments['DURABLE_SERVER_IMAGE'] ?? null);
        $this->assertSame($artifacts['server'], $assignments['DURABLE_SERVER_VERSION'] ?? null);
        $this->assertSame($artifacts['cli'], $assignments['DURABLE_WORKFLOW_CLI_VERSION'] ?? null);
        $this->assertSame('dw=='.$artifacts['cli'], $assignments['DURABLE_WORKFLOW_CLI_PIN'] ?? null);
        $this->assertSame($artifacts['sdk-python'], $assignments['DURABLE_WORKFLOW_PYTHON_SDK_VERSION'] ?? null);
        $this->assertSame($artifacts['sdk-rust'], $assignments['DURABLE_WORKFLOW_RUST_SDK_VERSION'] ?? null);
        $this->assertSame($artifacts['sdk-php'], $assignments['DURABLE_WORKFLOW_PHP_SDK_VERSION'] ?? null);
        $this->assertSame(
            'durable-workflow/sdk:'.$artifacts['sdk-php'].'@beta',
            $assignments['DURABLE_WORKFLOW_PHP_SDK_PIN'] ?? null,
        );
        $this->assertSame($artifacts['workflow'], $assignments['DURABLE_WORKFLOW_WORKFLOW_VERSION'] ?? null);
        $this->assertSame(
            'durable-workflow/workflow:'.$artifacts['workflow'].'@beta',
            $assignments['DURABLE_WORKFLOW_WORKFLOW_PIN'] ?? null,
        );
        $this->assertSame($artifacts['waterline'], $assignments['DURABLE_WORKFLOW_WATERLINE_VERSION'] ?? null);
        $this->assertSame(
            'durable-workflow/waterline:'.$artifacts['waterline'].'@beta',
            $assignments['DURABLE_WORKFLOW_WATERLINE_PIN'] ?? null,
        );
    }
    ...
    public function test_polyglot_smoke_metadata_covers_required_conformance_surfaces(): void
    {
        ...
        foreach ([
            ...
            'polyglot.python-to-php.binary-type-roundtrip',
            'polyglot.php-to-python.binary-type-roundtrip',
            'polyglot.rust.signal-query',
            'polyglot.rust-to-python.binary-type-roundtrip',
            'polyglot.python-to-rust.binary-type-roundtrip',
            'polyglot.rust-to-php.binary-type-roundtrip',
