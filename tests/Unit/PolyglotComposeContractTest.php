...
final class PolyglotComposeContractTest extends TestCase
{
    ...
    public function test_polyglot_server_artifact_defaults_to_current_public_image(): void
    {
        ...
        $this->assertStringContainsString('durableworkflow/server:0.2.151', $smokeShell);
        ...
        $this->assertStringContainsString('durableworkflow/server:0.2.151', $smokeDriver);
    ...
    public function test_polyglot_smoke_installs_published_cli_and_configures_waterline(): void
    {
        ...
        $this->assertStringContainsString('ARG DURABLE_WORKFLOW_PYTHON_SDK_VERSION=0.4.64', $dockerfile);
        ...
        $this->assertStringContainsString('ARG DURABLE_WORKFLOW_PYTHON_SDK_VERSION=0.4.64', $pythonWorkflowDockerfile);
        $this->assertStringContainsString(
            'durable-workflow==${DURABLE_WORKFLOW_PYTHON_SDK_VERSION}',
        ...
        $this->assertStringContainsString('DURABLE_WORKFLOW_PYTHON_SDK_VERSION:=0.4.64', $smokeShell);
        ...
        $this->assertSame(
            '${DURABLE_WORKFLOW_PYTHON_SDK_VERSION:-0.4.64}',
    ...
    private function assertServerImageDefaultIsCurrent(mixed $image): void
    {
        ...
        $this->assertTrue(
            version_compare($matches['version'], '0.2.151', '>='),
            sprintf('Expected durableworkflow/server default >= 0.2.151, got %s.', $image),
        );
    }
