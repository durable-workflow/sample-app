...
class SampleTeachingMaterialTest extends TestCase
{
    ...
    public function test_full_conformance_harness_is_public_and_names_required_surfaces(): void
    {
        ...
        $this->assertStringContainsString('default_server_image="durableworkflow/server:0.2.377"', $artifactResolver);
        ...
        $this->assertStringContainsString('default_cli_version="0.1.80"', $artifactResolver);
        $this->assertStringContainsString('latest_github_release_version durable-workflow/cli', $artifactResolver);
        $this->assertStringContainsString('default_python_sdk_version="0.4.86"', $artifactResolver);
        $this->assertStringContainsString('latest_pypi_version durable-workflow', $artifactResolver);
        $this->assertStringContainsString('default_workflow_version="2.0.0-alpha.203"', $artifactResolver);
        $this->assertStringContainsString('default_waterline_version="2.0.0-alpha.86"', $artifactResolver);
