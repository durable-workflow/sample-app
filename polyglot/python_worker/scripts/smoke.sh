...
: "${DURABLE_SERVER_IMAGE:=durableworkflow/server:0.2.132}"
: "${DURABLE_WORKFLOW_CLI_VERSION:=0.1.40}"
...
if [ -z "${DURABLE_WORKFLOW_PHP_SDK_VERSION:-}" ]; then
    ...
    : "${DURABLE_WORKFLOW_PHP_SDK_VERSION:=2.0.0-alpha.154}"
fi
if [ -z "${DURABLE_WORKFLOW_WATERLINE_VERSION:-}" ]; then
    ...
    : "${DURABLE_WORKFLOW_WATERLINE_VERSION:=2.0.0-alpha.50}"
fi
