...
REQUIRED_ENV = {
    ...
    "DURABLE_WORKFLOW_PYTHON_SDK_VERSION": "2.0.0-beta.16",
...
class ArtifactVersionFindingsTest(unittest.TestCase):
    ...
    def test_rust_artifact_metadata_uses_cargo_exact_requirement_syntax(self) -> None:
        versions = self.versions("2.0.0b16")
        versions["sdk-rust"] = "2.0.0-beta.16"
        rust = polyglot_smoke.artifact_metadata(versions)["sdk_rust"]

        self.assertEqual(
            "cargo add durable-workflow@=2.0.0-beta.16",
            rust["pin"],
        )
