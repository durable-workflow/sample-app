...
by running two scenarios end to end against one standalone server, with
real workers in different languages registered on the same task queues.
...
The Python-authored scenario is the language-symmetric reference:
...
│   └── scripts/
│       ├── smoke.sh                    drives both scenarios end to end
...
`php artisan app:polyglot-worker` command, which registers itself
against the standalone server and pumps workflow tasks through a
Fiber-based replay of the same class file.
...
docker compose up -d --build --wait \
  server python-activity-worker php-workflow-worker python-workflow-worker
...
Both scenarios use the `avro` codec by default — that is the v2
